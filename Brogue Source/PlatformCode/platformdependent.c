...
// twp
// saves every score from every game
// adds an additional line to the score archive file
// does not sort
void archiveScoreToFile(rogueHighScoresEntry theEntry) {
	long timeNow = (long) time(NULL);
	FILE *archiveFile;

	archiveFile = fopen("BrogueScoresArchive.txt", "a"); // does 'a'ppend work?

    fprintf(archiveFile, "%li\t%li\t%s\t%li\t%s\t%li\t%li\t%li\n", theEntry.score, timeNow, theEntry.description,
            theEntry.level_killed, theEntry.killed_by, (int)theEntry.had_amulet, theEntry.num_gems, (int)theEntry.was_victory);
