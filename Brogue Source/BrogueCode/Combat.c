...
boolean forceBoltHit(creature *defender, short caster_x, short caster_y) { // wand_of_force
    ...
    // actual difference between caster and target
    actualDiff[0] = defender->xLoc - caster_x;
    actualDiff[1] = defender->yLoc - caster_y;

    // 'clamped' or unit difference between caster and target (-1,0,+1)
    clampDiff[0] = clamp(actualDiff[0], -1, 1);
    clampDiff[1] = clamp(actualDiff[1], -1, 1);

    // allow straight vectors to occur more often by checking ratio of x to y
    // this way not all off-center blasts are automatically 45-degree
    if ((actualDiff[0] != 0) && (actualDiff[1] != 0)) {
        // neither offset is 0, so by default it will be a 45-degree shot
        ratioX = fabs((double) actualDiff[1] / actualDiff[0]);
        ratioY = fabs((double) actualDiff[0] / actualDiff[1]);
    ...
    newLoc[0] = defender->xLoc + clampDiff[0];
    newLoc[1] = defender->yLoc + clampDiff[1];
    ...
    if (canDirectlySeeMonster(defender)
        ...
        sprintf(buf, "%s flies backward with the force from the wand", monstName);
        buf[DCOLS] = '\0';
        combatMessage(buf, messageColorFromVictim(defender));
        autoID = true;
    }
    theBolt = boltCatalog[BOLT_BLINKING];
    theBolt.magnitude = max(1, weaponishEnchant + FLOAT_FUDGE);
    ...
    if (!(defender->bookkeepingFlags & MB_IS_DYING)
        ...
        && distanceBetween(oldLoc[0], oldLoc[1], defender->xLoc, defender->yLoc) < weaponForceDistance(weaponishEnchant)) {
