...
void gameOver(char *killedBy, boolean useCustomPhrasing) {
    ...
    if (numberOfMatchingPackItems(AMULET, 0, 0, false) > 0) {
        ...
        theEntry.had_amulet = 1;
    } else {
        theEntry.had_amulet = 0;
    }

    // more explicit record keeping
    theEntry.level_killed = rogue.depthLevel;
    strcpy(theEntry.killed_by, killedBy);
    theEntry.was_victory = 0;

    // figure out # of lumenstones
    short num_gems = 0;
    for (theItem = packItems->nextItem; theItem != NULL; theItem = theItem->nextItem) {
		if (theItem->category & GEM) {
			num_gems += theItem->quantity;
		}
    }
    theEntry.num_gems = num_gems;
...
void victory(boolean superVictory) {
    ...
    // twp add more record keeping
    theEntry.level_killed = rogue.depthLevel;
    strcpy(theEntry.killed_by, victoryVerb);
    theEntry.was_victory = 1;
    theEntry.had_amulet = 1;
    theEntry.num_gems = gemCount;
